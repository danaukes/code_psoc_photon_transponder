/***************************************************************************//**
* \file myUSB_pm.c
* \version 3.20
*
* \brief
*  This file provides Suspend/Resume APIs implementation.
*
********************************************************************************
* \copyright
* Copyright 2008-2016, Cypress Semiconductor Corporation.  All rights reserved.
* You may use this file only in accordance with the license, terms, conditions,
* disclaimers, and limitations in the end user license agreement accompanying
* the software package with which this file was provided.
*******************************************************************************/

#include "myUSB_pvt.h"
#include "myUSB_Dp.h"
#include "cyapicallbacks.h"

/***************************************
* Custom Declarations
***************************************/
/* `#START PM_CUSTOM_DECLARATIONS` Place your declaration here */

/* `#END` */


/***************************************
* Local data allocation
***************************************/

static myUSB_BACKUP_STRUCT  myUSB_backup;

#if (myUSB_DP_ISR_ACTIVE)
    /*******************************************************************************
    * Function Name: myUSB_DP_ISR
    ****************************************************************************//**
    *
    *  This Interrupt Service Routine handles DP pin changes for wake-up from
    *  the sleep mode.
    *
    *******************************************************************************/
    CY_ISR(myUSB_DP_ISR)
    {
    #ifdef myUSB_DP_ISR_ENTRY_CALLBACK
        myUSB_DP_ISR_EntryCallback();
    #endif /* (myUSB_DP_ISR_ENTRY_CALLBACK) */

        /* `#START DP_USER_CODE` Place your code here */

        /* `#END` */

        (void) myUSB_Dp_ClearInterrupt();
		
	#ifdef myUSB_DP_ISR_EXIT_CALLBACK
        myUSB_DP_ISR_ExitCallback();
    #endif /* (myUSB_DP_ISR_EXIT_CALLBACK) */
    }
#endif /* (myUSB_DP_ISR_ACTIVE) */


/*******************************************************************************
* Function Name: myUSB_SaveConfig
****************************************************************************//**
*
*  Saves the current user configuration.
*
* \reentrant
*  No.
*
*******************************************************************************/
void myUSB_SaveConfig(void) 
{
    /* Empty function added for the compatibility purpose. */
}


/*******************************************************************************
* Function Name: myUSB_RestoreConfig
****************************************************************************//**
*
*  Restores the current user configuration.
*
* \reentrant
*  No.
*
*******************************************************************************/
void myUSB_RestoreConfig(void) 
{
    if (myUSB_configuration != 0u)
    {
        myUSB_ConfigReg();
        myUSB_EpStateInit();
    }
}


/*******************************************************************************
* Function Name: myUSB_Suspend
****************************************************************************//**
*
*  This function prepares the USBFS component to enter low power mode. The 
*  interrupt on falling edge on Dp pin is configured to wakeup device when the 
*  host drives resume condition. The pull-up is enabled on the Dp line while 
*  device is in low power mode. The supported low power modes are Deep Sleep 
*  (PSoC 4200L) and Sleep (PSoC 3/ PSoC 5LP).
*  
*  *Note* For PSoC 4200L devices, this function should not be called before 
*  entering Sleep.
*  
*  *Note* After enter low power mode, the data which is left in the IN or OUT 
*  endpoint buffers is not restored after wakeup and lost. Therefore it should 
*  be stored in the SRAM for OUT endpoint or read by the host for IN endpoint 
*  before enter low power mode.
*
* \globalvars
*  myUSB_backup.enable:  modified.
*
* \reentrant
*  No.
*
*******************************************************************************/
void myUSB_Suspend(void) 
{
    uint8 enableInterrupts;

    enableInterrupts = CyEnterCriticalSection();

    if (0u != (myUSB_CR0_REG & myUSB_CR0_ENABLE))
    {
        /* USB block is enabled. */
        myUSB_backup.enableState = 1u;

    #if (myUSB_EP_MANAGEMENT_DMA)
        myUSB_Stop_DMA(myUSB_MAX_EP);
    #endif /* (myUSB_EP_MANAGEMENT_DMA) */

    #if (CY_PSOC4)
        /* Suspend enter sequence. */
        myUSB_POWER_CTRL_REG |= (myUSB_POWER_CTRL_SUSPEND |
                                            myUSB_POWER_CTRL_SUSPEND_DEL);

        /* Store state of USB regulator and disable it. */
        myUSB_backup.mode = (uint8)  (myUSB_CR1_REG & myUSB_CR1_REG_ENABLE);
        myUSB_CR1_REG    &= (uint32) ~myUSB_CR1_REG_ENABLE;
        
        /* Store SIE interrupt sources. Valid bits are 0 - 4. */
        myUSB_backup.intrSeiMask = (uint8) myUSB_INTR_SIE_MASK_REG;

    #else
        /* Ensure USB transmit enable is low (USB_USBIO_CR0.ten). - Manual Transmission - Disabled. */
        myUSB_USBIO_CR0_REG &= (uint8) ~myUSB_USBIO_CR0_TEN;
        CyDelayUs(myUSB_WAIT_REG_STABILITY_50NS);  /*~50ns delay. */

        /* Disable the USBIO by asserting PM.USB_CR0.fsusbio_pd_n(Inverted) and pd_pullup_hv(Inverted) high. */
        myUSB_PM_USB_CR0_REG &= (uint8) ~(myUSB_PM_USB_CR0_PD_N |
                                                     myUSB_PM_USB_CR0_PD_PULLUP_N);

        /* Disable the SIE. */
        myUSB_CR0_REG &= (uint8) ~myUSB_CR0_ENABLE;

        CyDelayUs(myUSB_WAIT_REG_STABILITY_50NS);  /* ~50ns delay. */
        /* Store mode and disable VRegulator. */
        myUSB_backup.mode = (uint8) (myUSB_CR1_REG & myUSB_CR1_REG_ENABLE);
        myUSB_CR1_REG    &= (uint8) ~myUSB_CR1_REG_ENABLE;

        CyDelayUs(myUSB_WAIT_REG_STABILITY_1US); /* min 0.5us delay required. */

        /* Disable the USBIO reference by setting PM.USB_CR0.fsusbio_ref_en.*/
        myUSB_PM_USB_CR0_REG &= (uint8) ~myUSB_PM_USB_CR0_REF_EN;

        /* Switch DP and DM terminals to GPIO mode and disconnect 1.5k pull-up. */
        myUSB_USBIO_CR1_REG |= myUSB_USBIO_CR1_IOMODE;

        /* Disable USBFS block. */
        /* Clear power active and standby mode templates: disable USB block. */
        myUSB_PM_ACT_CFG_REG  &= (uint8) ~myUSB_PM_ACT_EN_FSUSB;
        myUSB_PM_STBY_CFG_REG &= (uint8) ~myUSB_PM_STBY_EN_FSUSB;

        CyDelayUs(myUSB_WAIT_REG_STABILITY_1US); /* min 0.5us delay required. */
    #endif /* (CY_PSOC4) */
    }
    else
    {
        myUSB_backup.enableState = 0u;
    }

    CyExitCriticalSection(enableInterrupts);

#if (myUSB_DP_ISR_ACTIVE)
    /* Clear active mode Dp interrupt source history. */
    (void) myUSB_Dp_ClearInterrupt();
    CyIntClearPending(myUSB_DP_INTC_VECT_NUM);

    CyIntEnable      (myUSB_DP_INTC_VECT_NUM);
#endif /* (myUSB_DP_ISR_ACTIVE). */
}


/*******************************************************************************
* Function Name: myUSB_Resume
****************************************************************************//**
*
*  This function prepares the USBFS component for active mode operation after 
*  exit low power mode. It restores the component active mode configuration such 
*  as device address assigned previously by the host, endpoints buffer and disables 
*  interrupt on Dp pin.
*  The supported low power modes are Deep Sleep (PSoC 4200L) and Sleep 
*  (PSoC 3/ PSoC 5LP).
*  
*  *Note* For PSoC 4200L devices, this function should not be called after 
*  exiting Sleep.
*  
*  *Note* To resume communication with the host, the data endpoints must be 
*  managed: the OUT endpoints must be enabled and IN endpoints must be loaded 
*  with data. For DMA with Automatic Buffer Management, all endpoints buffers 
*  must be initialized again before making them available to the host.
*
*
* \globalvars
*  myUSB_backup - checked.
*
* \reentrant
*  No.
*
*******************************************************************************/
void myUSB_Resume(void) 
{
    uint8 enableInterrupts;

    enableInterrupts = CyEnterCriticalSection();

    if (0u != myUSB_backup.enableState)
    {
    #if (myUSB_DP_ISR_ACTIVE)
        CyIntDisable(myUSB_DP_INTC_VECT_NUM);
    #endif /* (myUSB_DP_ISR_ACTIVE) */

    #if (CY_PSOC4)
        /* Enable clock to USB IP. */
        myUSB_USB_CLK_EN_REG |= myUSB_USB_CLK_CSR_CLK_EN;

        /* Restore arbiter configuration for DMA transfers. */
        #if (myUSB_EP_MANAGEMENT_DMA)
            #if (myUSB_ARB_ISR_ACTIVE)
                /* Enable ARB EP interrupt sources. */
                myUSB_ARB_INT_EN_REG = myUSB_DEFAULT_ARB_INT_EN;
            #endif /* (myUSB_EP_MANAGEMENT_DMA) */

            /* Configure arbiter for Manual or Auto DMA operation and clear 
            * configuration completion. 
            */
            myUSB_ARB_CFG_REG = myUSB_DEFAULT_ARB_CFG;
        #endif /* (myUSB_EP_MANAGEMENT_DMA) */

        /* Restore level (hi, lo, med) for each interrupt source. */
        myUSB_INTR_LVL_SEL_REG = myUSB_DEFAULT_INTR_LVL_SEL;
        
        /* Store SIE interrupt sources. */
        myUSB_INTR_SIE_MASK_REG = (uint32) myUSB_backup.intrSeiMask;

        /* Set EP0.CR: ACK Setup, NAK IN/OUT. */
        myUSB_EP0_CR_REG = myUSB_MODE_NAK_IN_OUT;

        /* Restore data EP1-8 configuration. */
        myUSB_RestoreConfig();

        /* Restore state of USB regulator and wait until it supples stable power. */
        myUSB_CR1_REG |= myUSB_backup.mode;
        CyDelayUs(myUSB_WAIT_VREF_STABILITY);

        /* Suspend exit sequence. */
        myUSB_POWER_CTRL_REG &= (uint32) ~myUSB_POWER_CTRL_SUSPEND;
        CyDelayUs(myUSB_WAIT_SUSPEND_DEL_DISABLE);
        myUSB_POWER_CTRL_REG &= (uint32) ~myUSB_POWER_CTRL_SUSPEND_DEL;

    #else
        /* Set power active and standby mode templates: enable USB block. */
        myUSB_PM_ACT_CFG_REG  |= myUSB_PM_ACT_EN_FSUSB;
        myUSB_PM_STBY_CFG_REG |= myUSB_PM_STBY_EN_FSUSB;

        /* Enable core clock. */
        myUSB_USB_CLK_EN_REG |= myUSB_USB_CLK_ENABLE;

        /* Enable the USBIO reference by setting PM.USB_CR0.fsusbio_ref_en.*/
        myUSB_PM_USB_CR0_REG |= myUSB_PM_USB_CR0_REF_EN;

        /* The reference is available ~40us after power restored. */
        CyDelayUs(myUSB_WAIT_VREF_RESTORE);
        /* Restore state of USB regulator and wait until it supples stable power. */
        myUSB_CR1_REG |= myUSB_backup.mode;
        CyDelayUs(myUSB_WAIT_VREF_STABILITY);   /*~50ns delay. */

        /* Enable USBIO. */
        myUSB_PM_USB_CR0_REG |= myUSB_PM_USB_CR0_PD_N;
        CyDelayUs(myUSB_WAIT_PD_PULLUP_N_ENABLE);
        /* Set the USBIO pull-up enable. */
        myUSB_PM_USB_CR0_REG |= myUSB_PM_USB_CR0_PD_PULLUP_N;

        /* Restore arbiter configuration for DMA transfers. */
        #if (myUSB_EP_MANAGEMENT_DMA)
            #if (myUSB_ARB_ISR_ACTIVE)
                /* Enable ARB EP interrupt sources. */
                myUSB_ARB_INT_EN_REG = myUSB_DEFAULT_ARB_INT_EN;
            #endif /* (myUSB_EP_MANAGEMENT_DMA) */

            /* Configure arbiter for Manual or Auto DMA operation and clear 
            * configuration completion. 
            */
            myUSB_ARB_CFG_REG = myUSB_DEFAULT_ARB_CFG;
        #endif /* (myUSB_EP_MANAGEMENT_DMA) */

        /* Set EP0.CR: ACK Setup, STALL IN/OUT. */
        myUSB_EP0_CR_REG = myUSB_MODE_STALL_IN_OUT;

        /* Enable the USB IP to respond to USB traffic with the last address. */
        myUSB_CR0_REG |= myUSB_CR0_ENABLE;
        CyDelayCycles(myUSB_WAIT_CR0_REG_STABILITY);

        /* Enable D+ pull-up and keep USB control on IO. */
        myUSB_USBIO_CR1_REG = myUSB_USBIO_CR1_USBPUEN;

        /* Restore data EP1-8 configuration. */
        myUSB_RestoreConfig();
    #endif /* (CY_PSOC4) */
    }

    CyExitCriticalSection(enableInterrupts);
}


/* [] END OF FILE */
